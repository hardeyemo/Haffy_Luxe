...
export default function Navbar() {
  ...
  return (
    <nav className="bg-white shadow-md sticky top-0 z-50">
        ...
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="hidden md:flex items-center space-x-8 font-semibold">
            ...
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="hover:text-green-700 transition cursor-pointer flex items-center gap-1"
              >
                Products
              </button>

              {productsOpen && (
                <div className="absolute top-12 left-0 w-72 bg-white shadow-xl rounded-lg py-4 z-50 grid grid-cols-2 gap-4 p-4 border">
                  {productCategories.map((cat) => (
                    <Link
                      key={cat.name}
                      href={cat.href}
                      className="flex flex-col items-center gap-2 hover:bg-gray-100 p-2 rounded-lg transition transform hover:scale-105"
                      onClick={() => setProductsOpen(false)} // close after click
                    >
                      <Image
                        src={cat.image}
                        alt={cat.name}
          ...
          <div className="flex items-center space-x-4">
            <Link href="/checkout" className="relative">
              <FiShoppingCart size={24} className="cursor-pointer text-gray-700 hover:text-green-700 transition" />
              {totalQuantity > 0 && (
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div className="md:hidden bg-white shadow-md px-2 pt-2 pb-4 space-y-1">
            ...
            <div className="pl-4 grid grid-cols-2 gap-2">
              {productCategories.map((cat) => (
                <Link
                  key={cat.name}
                  href={cat.href}
                  className="flex flex-col items-center hover:bg-gray-50 p-2 rounded transition"
